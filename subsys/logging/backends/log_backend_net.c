...
#define CONFIG_LOG_BACKEND_NET_TLS
...
static int do_net_init(struct log_backend_net_ctx *ctx)
{
	...
	if (ctx->is_tcp) {
		...
		ret = zsock_setsockopt(ctx->sock, SOL_TLS, TLS_SEC_TAG_LIST, sec_tls_tags,
		                       sizeof(sec_tls_tags));
		if (ret < 0) {
		...
		ret = zsock_setsockopt(ctx->sock, SOL_TLS, TLS_HOSTNAME, CONFIG_LOG_BACKEND_NET_TLS_PEER_NAME,
